...
// Correction: Déplace FADE_UP_VARIANTS en dehors du composant si non dépendant de props/state
  ...
  return (
    <>
      <Head>
        ...
        <meta name="description" content={t('home_page_meta_description', 'NocaFLOW est l\'OS client tout-en-un pour les professionnels modernes. Arrêtez d\'envoyer des PDFs, commencez à conclure des affaires.')} />
      </Head>
      ...
      >
        ...
        <div className="relative z-10 flex flex-col items-center">
            <motion.div variants={FADE_UP_VARIANTS} className="text-center">
                <h1 className="text-5xl md:text-7xl font-bold tracking-tighter animated-gradient-text dark-gradient-text">{t('main_headline', 'Le Système d\'Exploitation Client')}</h1>
                <h1 className="text-5xl md:text-7xl font-bold tracking-tighter animated-gradient-text light-gradient-text mt-2 md:mt-4">{t('main_headline_2', 'Tout-en-Un pour Votre Business.')}</h1>
            </motion.div>
            <motion.p variants={FADE_UP_VARIANTS} className="mt-6 text-xl md:text-2xl text-slate-300 max-w-3xl" dangerouslySetInnerHTML={{ __html: t('sub_headline') }} />
      ...
      <section className="py-20 px-4 bg-gray-900/50">
        <div className="max-w-7xl mx-auto text-center">
          <h2 className="text-4xl font-bold text-white mb-8">{t('features_overview', 'Aperçu des Fonctionnalités')}</h2>
          <p className="text-lg text-slate-400 max-w-3xl mx-auto">{t('features_overview_description', 'Découvrez comment NocaFLOW simplifie votre gestion quotidienne avec des outils puissants et intégrés.')}</p>
        </div>
      </section>

      <section className="py-20 px-4">
        <div className="max-w-7xl mx-auto text-center">
          <h2 className="text-4xl font-bold text-white mb-8">{t('testimonials', 'Ce que nos utilisateurs disent')}</h2>
        </div>
      </section>
    </>
  );
