...
export default function Navbar({ onLoginClick, onRegisterClick, onOpenCalculator }) {
  ...
  // Fallback sûr pour l'URL de l'avatar
  const avatarUrl = isGuestMode ? '/images/avatars/avatarguest.jpg' : (user?.photoURL || '/images/avatars/yves.jpg');
  ...
  useEffect(() => {
    setIsOpen(false); // Ferme le menu mobile sur changement de route
  }, [asPath]);
