// components/dashboard/TaskItem.js
    ...
    export const TaskItem = ({ task, onToggle, onDelete, onEdit, t }) => { 
        ...
        return(
            <div className="futuristic-card flex items-center gap-3 p-2 rounded-lg transition-colors duration-200 hover:bg-color-bg-hover group">
                <input 
                  type="checkbox" 
                ...
                />
                <div className="flex-shrink-0">{priorityMap[task.priority]?.icon || priorityMap.normal.icon}</div>
                <div className="flex-grow min-w-0">
                    <p className={`truncate ${task.completed ? 'line-through text-color-text-tertiary' : 'text-color-text-primary'}`}>{task.title}</p>
