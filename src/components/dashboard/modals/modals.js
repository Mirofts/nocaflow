                ...
                {eventTypeOptions.map(option => (
                    <button
                        ...
                        className={`flex-1 p-2 rounded-md transition-all flex items-center justify-center gap-2 text-sm font-semibold
                                    ${type === option.id ? 'bg-pink-500 text-white shadow-lg' : 'bg-slate-700 text-slate-300 hover:bg-slate-600'}`}
                    >
...
const AvatarEditModal = ({ onClose, t, isGuestMode, onUpdateGuestAvatar }) => {
    ...
    if (isGuestMode && typeof window !== 'undefined') {
        ...
        if (guestData.user?.photoURL) {
            ...
            setPreviewUrl('/images/avatars/avatarguest.jpg'); // Default for guests
        }
    } else if (user?.photoURL) {
        setPreviewUrl(user.photoURL);
    } else {
        setPreviewUrl('/images/default-avatar.jpg'); // Default for logged-in users without avatar
    }
    }, [user, isGuestMode]);
    ...
    if (isGuestMode) {
        uploadAndSaveAvatar(previewUrl); // For guest, directly use previewUrl (local file URL)
    } else {
        if (!user || !user.uid) {
            setError(t('user_not_connected', "Utilisateur non connecté."));
            return;
...
const TeamMemberModal = ({ mode, member, onSave, onDelete, onClose, t }) => {
    ...
    const [avatar, setAvatar] = useState(member?.avatar || '/images/default-avatar.jpg');
                ...
                <div className="flex-grow overflow-y-auto custom-scrollbar pr-2 space-y-4">
                    {chatHistory.map((msg, index) => (
                        <div key={index} className={`flex gap-3 ${msg.sender === 'Vous' ? 'justify-end' : 'justify-start'}`}>
                            {msg.sender !== 'Vous' && (
                                // MODIFICATION ICI: Utilise default-avatar.jpg comme fallback pour le membre
                                <Image src={member?.avatar || '/images/default-avatar.jpg'} alt={member?.name || 'Partenaire'} width={32} height={32} className="rounded-full object-cover flex-shrink-0" />
                            )}
                            ...
                            {msg.sender === 'Vous' && (
                                // MODIFICATION ICI: Utilise default-avatar.jpg pour l'utilisateur
                                <Image src="/images/default-avatar.jpg" alt="Vous" width={32} height={32} className="rounded-full object-cover flex-shrink-0" />
                            )}
                        </div>
                    ))}
                    <div ref={chatEndRef} />
...
const AssignTaskProjectDeadlineModal = ({ member, onClose, t, allStaffMembers = [], userUid, currentUserName, onAddTask }) => {
    ...
    // Initialize assignedTo with the provided member's name or current user's name
    const [assignedTo, setAssignedTo] = useState(member?.name || currentUserName || '');
    const [loading, setLoading] = useState(false);

    // Filter out the current user from assignable staff if they are already selected by default
    const assignableStaff = useMemo(() => {
        // Ensure allStaffMembers is an array, map to include uid for proper comparison
    ...
    // Update assignedTo if a specific member is passed and is different from current user
    useEffect(() => {
    ...
    const handleSubmit = async (e) => {
        ...
        if (assignmentType === 'task') {
            ...
            // Assuming onAddTask expects only the task title string
...
const AddDeadlineModal = ({ onSave, onClose, t }) => {
    ...
    return (
        <ModalWrapper onClose={onClose} size="max-w-md">
            <h2 className="text-2xl font-bold text-white mb-6 text-center flex items-center justify-center gap-2">
                <svg xmlns="http://www.w3.org/2000/svg" width="24" height="24" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round"><rect width="18" height="18" x="3" y="4" rx="2" ry="2"/><line x1="16" x2="16" y1="2" y2="6"/><line x1="8" x2="8" y1="2" y2="6"/><line x1="3" y1="10" y2="10"/></svg>
