...
const TodoList = ({ todos, loading, onAdd, onToggle, onDelete, onEdit, t }) => {
  ...
  return (
    ...
    } title={t('todo_list_title', 'Liste de Tâches')} className="h-full"> {/* Removed bg-blue-900/20 */}
      {loading ? (
        ...
        <div className="h-full flex flex-col">
          <div className="flex-grow space-y-1 overflow-y-auto max-h-80 custom-scrollbar pr-2">
            {Array.isArray(todos) && todos.length > 0
              ? todos.map(task => <TaskItem key={task.id} task={task} onToggle={onToggle} onDelete={onDelete} onEdit={onEdit} t={t} />) {/* PASSED T PROP */}
