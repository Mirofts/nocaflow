...
// Assurez-vous que le chemin d'importation de DashboardCard est correct
// Si DashboardCard est dans le même dossier 'dashboard', c'est './DashboardCard'
// Si DashboardCard est dans un sous-dossier, ajustez le chemin en conséquence.
import { DashboardCard } from './DashboardCard'; // <-- Vérifiez ce chemin !
...
const SingleTimeAlertCard = ({ type, title, dateTime, icon, pulseColorClass, openCreateModal, onCardClick, alertData, t }) => {
    ...
    return (
        ...
        >
            ...
            <h4 className={`text-center font-extrabold mb-1 ${isDarkMode ? 'text-indigo-400' : 'text-indigo-700'}`} style={{ fontSize: '1rem' }}> {/* Reduced text-xl to text-lg equivalent */}
                {title}
            </h4>
            <p className={`text-center font-black ${timeLeft.overdue ? 'text-red-500' : (isUrgent ? 'text-orange-400' : (isDarkMode ? 'text-white' : 'text-gray-900'))} mb-2 leading-tight`} style={{ fontSize: '1.5rem' }}> {/* Reduced text-3xl to text-2xl equivalent */}
                {displayTime}
            </p>
            <p className="text-center text-gray-400 dark:text-gray-500 mb-4 font-medium" style={{ fontSize: '0.875rem' }}> {/* Reduced text-base to text-sm equivalent */}
            ...
            <span className="mt-2 font-semibold text-gray-500 dark:text-gray-400" style={{ fontSize: '0.75rem' }}> {/* Reduced text-sm to text-xs equivalent */}
